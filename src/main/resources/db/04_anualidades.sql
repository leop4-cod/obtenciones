...
CREATE TABLE IF NOT EXISTS anualidades (
    ...
    valor_pagado_recargo       DECIMAL(10,2) NULL,
    numero_comprobante_recargo VARCHAR(150)  NULL,
...
-- ── Migración: agregar columnas de recargo si la tabla ya existía ──
DROP PROCEDURE IF EXISTS agregar_columnas_recargo_anualidades;
DELIMITER $$
CREATE PROCEDURE agregar_columnas_recargo_anualidades()
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'anualidades' AND COLUMN_NAME = 'valor_pagado_recargo'
    ) THEN
        ALTER TABLE anualidades ADD COLUMN valor_pagado_recargo DECIMAL(10,2) NULL AFTER numero_comprobante;
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'anualidades' AND COLUMN_NAME = 'numero_comprobante_recargo'
    ) THEN
        ALTER TABLE anualidades ADD COLUMN numero_comprobante_recargo VARCHAR(150) NULL AFTER valor_pagado_recargo;
    END IF;
END$$
DELIMITER ;
CALL agregar_columnas_recargo_anualidades();
DROP PROCEDURE IF EXISTS agregar_columnas_recargo_anualidades;
